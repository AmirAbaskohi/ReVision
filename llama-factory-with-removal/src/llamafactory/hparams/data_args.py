...
class DataArguments:
    ...
    image_token_removal_enabled: bool = field(
        default=False,
        metadata={"help": "Whether to enable temporal patch removal for image tokens during training."},
    )
    image_token_removal_threshold: float = field(
        default=0.5,
        metadata={"help": "Threshold for dropping patches based on saliency scores (0.0-1.0). Patches with lower scores are dropped."},
    )
    ...
    def __post_init__(self):
        ...
        if self.image_token_removal_enabled:
            if not 0.0 <= self.image_token_removal_threshold <= 1.0:
                raise ValueError(
                    f"image_token_removal_threshold must be in range [0.0, 1.0], got {self.image_token_removal_threshold}"
                )
