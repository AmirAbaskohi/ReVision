...
def configure_patch_removal(model: "PreTrainedModel", data_args: "DataArguments") -> None:
    r"""Configure temporal patch removal for ReVision training.
    
    Patches the model's forward method to apply temporal patch removal after vision encoding.
    Patches are dropped based on temporal similarity between consecutive images.
    """
    if not data_args.image_token_removal_enabled:
        return
    ...
    logger.info_rank0(
        f"Configuring temporal patch removal (threshold={data_args.image_token_removal_threshold})"
    )
    
    model_type = getattr(model.config, "model_type", None)
    if model_type not in COMPOSITE_MODELS:
        logger.warning_rank0(f"Patch removal not supported for model type: {model_type}")
        return
    
    # Store configuration in model for access during forward pass
    patch_removal_config = {
        "enabled": data_args.image_token_removal_enabled,
        "threshold": data_args.image_token_removal_threshold,
        "aggregation": "mean",
    }
    model._patch_removal_config = patch_removal_config
    model._patch_removal_enabled = True
    
    # Store reference to original forward
    original_forward = model.forward
    
    def forward_with_patch_removal(self, *args, **kwargs):
        """Forward pass with temporal patch removal for vision tokens."""
        
        # Check if patch removal is enabled and we have the required data
        if (
            getattr(self, "_patch_removal_enabled", False)
            and "pixel_values" in kwargs
            and "image_grid_thw" in kwargs
        ):
            pixel_values = kwargs.get("pixel_values")
            image_grid_thw = kwargs.get("image_grid_thw")
            
            if pixel_values is not None and image_grid_thw is not None:
                # Create mm_inputs dict for patch removal
                mm_inputs = {
                    "pixel_values": pixel_values,
                    "image_grid_thw": image_grid_thw,
                }
                
                # Get batch image lengths if available
                batch_imglens = kwargs.get("batch_imglens", None)
                
                # Apply patch removal
                mm_inputs = apply_patch_removal_to_mm_input(
                    mm_inputs,
                    batch_imglens or [1],
                    self._patch_removal_config,
                )
                
                # Update kwargs with processed embeddings and grid
                kwargs["pixel_values"] = mm_inputs["pixel_values"]
                kwargs["image_grid_thw"] = mm_inputs["image_grid_thw"]
        
        # Call original forward with potentially modified mm_inputs
        return original_forward(*args, **kwargs)
    
    # Replace the forward method
    model.forward = MethodType(forward_with_patch_removal, model)
    logger.info_rank0("Successfully patched model forward for temporal patch removal.")
